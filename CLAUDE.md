...
Two exceptions. [tools/](tools/) is *about* the framework, not part of it: a linter that checks the invariants these documents promise each other, its mutation tests, the tests of the skill scripts, and `tools/asef_eval.py`, the harness that runs `examples/scenarios.md` against any agent. It is never loaded into an agent's context. [skills/asef/](skills/asef/) is the third activation method: an Agent Skills file, `skills/asef/SKILL.md` (loaded into context when `/asef` is invoked) plus two scripts that are executed, never loaded. Run all of them before committing any change to a framework document:
...
python3 tools/test_asef_eval.py    # the scenario harness keeps its promise
...
The folder is designed to be dropped into a target project as `asef/` and activated with the invocation text in `prompt universale ASEF.txt` (written in Italian; the framework documents themselves are in English — keep that split). `ASEF universal prompt.txt` is its English translation for users who do not write Italian: line for line the same prompt, never a variant. Change both together; the linter fails when their versions, framework pointers or request-block shape diverge, and `skills/asef/scripts/asef_prompt.py` holds the field labels of each language in `LABELS`. That file ends with a fill-in request block the user completes before pasting; its reading rules (an empty field is a gap, filled first-party fields already satisfy the demand exemption, constraints and non-goals are binding) are part of the contract, not decoration.
...
- **Bumping the kernel means bumping three files.** `asef.version` in `ASEF.md`, the `kernel vX.Y` line and the bootstrap version in both activation prompts, and a new top entry in `CHANGELOG.md`. The linter fails on any mismatch; the activation prompt defers to the kernel when it finds one.
- **A release restates nothing.** `.github/workflows/release.yml` publishes it, on a pushed version tag or on manual dispatch, and `tools/release_notes.py` derives tag, title and notes from `ASEF.md` and `CHANGELOG.md`. The leading `v` is optional because this repository's history carries both spellings: a pushed tag is published exactly as pushed, and only a manual run picks the preferred `vX.Y.Z` form, after checking whether a release already exists under an equivalent spelling. Do not make one spelling canonical without migrating the tags that already exist. It refuses to release a version the kernel does not declare or the changelog does not describe, so releasing is a consequence of the version bump, never a second place to describe the work. Never hand-write release notes: fix the changelog entry and republish.
- **`prompt universale ASEF.txt` loads the kernel, it does not mirror it.** It carries only what cannot be discovered before `ASEF.md` is read: bootstrap order, contract pointers, a capability declaration, the label-language rule, and the user request template (including the release authorizations `ship` honours). The first-output block is owned by `ROUTER.md` Output so every activation method emits it; the prompt only points to it. Rules owned by a kernel file are referenced there, never restated — re-summarising the gap policy or the fan-out mechanics in this file is the duplication the framework forbids. Update it when defaults, the runtime order, the traits, the risk classes or the review axes change in `ASEF.md`. `templates/AGENTS.template.md` is the same idea at minimum size: a pointer to the kernel, never a second copy of its rules.
- **The skill activates, it does not restate.** `skills/asef/SKILL.md` locates the framework, maps the user's words onto the request block through `skills/asef/scripts/asef_prompt.py` (which reads `prompt universale ASEF.txt` at runtime and fills only that block), and executes the result from Bootstrap step 1. Route, stack and questions stay with the kernel; the skill never pre-classifies a project, invents context for an empty field, or grants a release authorization the user did not state. The linter requires its name to be `asef`, its pointers to the kernel files and the builder, a 1,500-token ceiling, and no single-home rule inside it. Occasional commands (`init`, `upgrade`, `doctor`, `status`, installation) live in `skills/asef/references/commands.md`, loaded only when invoked, under its own 800-token ceiling: keep the common path in `skills/asef/SKILL.md` and nothing else. Changing the request block's fields means checking the builder's field mapping and `tools/test_asef_skill.py`. Its frontmatter follows the Agent Skills specification (only `name`, `description`, `license`, `compatibility`, `metadata`, `allowed-tools` at top level; agent-specific keys go under `metadata`), or strict agents reject the skill; the linter enforces it.
- **The runtime set has one home.** `RUNTIME_FILES` and `RUNTIME_DIRS` in `skills/asef/scripts/asef_prompt.py` list what an agent needs in a target project; `init`, `init --upgrade`, `install.py --bundle-framework` and the release archives copy exactly that, so maintainer files (this `CLAUDE.md`, `tools/`, CI) never reach a project where agents would read them as rules. Adding a runtime file or folder means adding it there; `tools/test_asef_skill.py` fails when a copied document names a framework file the copy lacks.
- **`examples/scenarios.md` is the evaluation contract.** `tools/asef_eval.py` parses its table (Fixture, Request, Expected behavior split on `;`, Fail if split on `;`, a leading route name checked against `STATE.md`); keep that shape when adding a case. The harness drives no agent: it prepares, judges records and aggregates, so it stays agent-neutral. Built-in fixtures live in `examples/fixtures/<CASE>/` (`base/` committed, `dirty/` copied over uncommitted); `tools/test_asef_eval.py` checks each still reproduces its case.
- **The capability fallbacks in `CONTEXT-MANAGER.md` are the shared portability contract.** A missing capability (no subagents, no web, no file writes, no execution, no browser, no git) changes the path, never the gates. Keep the activation prompt pointing to those fallbacks; no activation method may silently require a capability.
