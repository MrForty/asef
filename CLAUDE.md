...
- **`examples/scenarios.md` is the evaluation contract.** `tools/asef_eval.py` parses its table (Fixture, Request, Expected behavior split on `;`, Fail if split on `;`, a leading route name checked against `STATE.md`); keep that shape when adding a case. The harness drives no agent: it prepares, judges records and aggregates, so it stays agent-neutral.
