...
Run `python3 SCRIPT scan` from the project root (`python` on Windows): it prints the framework root, its versions and the detected artifacts. When it reports `upgrade_available: true`, mention it once in the first output; never upgrade unasked. Exit code 2 means no framework: follow `init` in `references/commands.md`; do not proceed without a framework.
...
| `init`, `upgrade`, `doctor`, `status` | Read `references/commands.md` and follow that command's row; stop |
